...
public class SimulationController {
	...
	public String simulation(@ModelAttribute @Validated SimulationForm form,
		...
		//ラムダ式
		results.stream().forEach(e -> e.setCalculated(servise.calculate(form.getBaseDate(), e.getFormula())));
	...
	public String delete(@PathVariable String dateId, Model model) {
		servise.delete(dateId);
		model.addAttribute("simulation", new SimulationForm());
		return "simulation";
	}
